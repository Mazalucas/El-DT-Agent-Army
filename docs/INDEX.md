# Índice de Documentación: Agents_Army

## 📖 Guía Rápida

Esta documentación está organizada para guiarte desde la instalación hasta el uso avanzado.

## 🚀 Para Empezar

### 1. [QUICK_START.md](QUICK_START.md) - ⚡ Inicio Rápido
**Empieza aquí** - Guía rápida en 5 minutos.

### 2. [REQUIREMENTS.md](REQUIREMENTS.md) - Requisitos
Verifica que tienes todo lo necesario.

### 3. [API_KEYS_CONFIG.md](API_KEYS_CONFIG.md) - ⚠️ Configuración de API Keys
**Importante**: Cómo configurar API keys de LLMs y MCP.

### 4. [USER_GUIDE.md](USER_GUIDE.md) - Guía de Usuario
Guía completa con ejemplos y casos de uso.

## 📚 Documentación Técnica

### Arquitectura y Diseño
- **[ARCHITECTURE.md](ARCHITECTURE.md)** - Arquitectura del sistema
- **[PROTOCOL.md](PROTOCOL.md)** - Protocolo de comunicación
- **[SPECIFICATIONS_V2.md](SPECIFICATIONS_V2.md)** - Especificaciones técnicas
- **[ROLES.md](ROLES.md)** - Roles y responsabilidades
- **[PROJECT_STRUCTURE_SEPARATION.md](PROJECT_STRUCTURE_SEPARATION.md)** - Separación DT vs Proyectos
- **[DT_COMMUNICATION_GUIDELINES.md](DT_COMMUNICATION_GUIDELINES.md)** - Guías de comunicación del DT

### Integración y Uso
- **[INTEGRATION.md](INTEGRATION.md)** - Guía de integración
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** - Solución de problemas
- **[FAQ.md](FAQ.md)** - Preguntas frecuentes

### Operaciones
- **[TESTING_STRATEGY.md](TESTING_STRATEGY.md)** - Estrategia de testing
- **[DEPLOYMENT.md](DEPLOYMENT.md)** - Deployment (futuro)
- **[SECURITY.md](SECURITY.md)** - Seguridad (futuro)
- **[MONITORING.md](MONITORING.md)** - Observabilidad (futuro)
- **[COST_MANAGEMENT.md](COST_MANAGEMENT.md)** - Gestión de costos (futuro)

## 📁 Documentación de Referencia

La documentación histórica ha sido eliminada para mantener solo lo esencial.

## 🔍 Búsqueda Rápida

### ¿Cómo...?
- **Instalar**: [INSTALLATION.md](docs/INSTALLATION.md)
- **Configurar API keys**: [API_KEYS_CONFIG.md](API_KEYS_CONFIG.md)
- **Usar el framework**: [USER_GUIDE.md](USER_GUIDE.md)
- **Integrar en mi proyecto**: [INTEGRATION.md](INTEGRATION.md)
- **Resolver problemas**: [TROUBLESHOOTING.md](TROUBLESHOOTING.md)

### ¿Qué es...?
- **Un agente**: [ROLES.md](ROLES.md#roles-principales)
- **El protocolo**: [PROTOCOL.md](PROTOCOL.md#visión-general)
- **La arquitectura**: [ARCHITECTURE.md](ARCHITECTURE.md#visión-general)
- **El DT**: [SPECIFICATIONS_V2.md](SPECIFICATIONS_V2.md#el-dt-director-técnico)
- **Separación de proyectos**: [PROJECT_STRUCTURE_SEPARATION.md](PROJECT_STRUCTURE_SEPARATION.md)
- **Comunicación del DT**: [DT_COMMUNICATION_GUIDELINES.md](DT_COMMUNICATION_GUIDELINES.md)

## 📋 Orden Recomendado de Lectura

### Para Usuarios Nuevos
1. [REQUIREMENTS.md](REQUIREMENTS.md)
2. [INSTALLATION.md](docs/INSTALLATION.md)
3. [API_KEYS_CONFIG.md](API_KEYS_CONFIG.md)
4. [USER_GUIDE.md](USER_GUIDE.md)

### Para Desarrolladores
1. [ARCHITECTURE.md](ARCHITECTURE.md)
2. [PROTOCOL.md](PROTOCOL.md)
3. [INTEGRATION.md](INTEGRATION.md)
4. [SPECIFICATIONS_V2.md](SPECIFICATIONS_V2.md)

### Para DevOps
1. [DEPLOYMENT.md](DEPLOYMENT.md)
2. [SECURITY.md](SECURITY.md)
3. [MONITORING.md](MONITORING.md)
4. [COST_MANAGEMENT.md](COST_MANAGEMENT.md)

---

**Última actualización**: Enero 2025
